...
$(document).ready(function () {
    ...
    const plavideoDescription = $('#video-description')
    ...
    function onVideoCardClickHandler(e) {
        ...
        // find the required video data movject and store it in videoObject variable 
        for (let counter = 0; counter < videoPlaySectionData.length; counter++)     {
            ...
            if (currentVideoData.id === videoId) {
                videoObj = currentVideoData;
                break;
            }
        }
        if (videoObj) {
            player.attr(`src`, playerBaseUrl + videoObj.vimeoId);
            videoViews.text(videoObj.views / 1000 + 'k')
            videoTitle.text(videoObj.title)
            plavideoDescription.text(videoObj.description)

            if(videoObj.isLiked == true){
            ...
            if(videoObj.isSaved == true){
                saveIcon.removeClass('far')
                saveIcon.addClass('fas')
            }else{
                saveIcon.removeClass('fas')
                saveIcon.addClass('far')
            }

        }
        // for border 
        const allCards = $('.playlist-card')         // define here bcz outside this handler there is not 10 cards
        for (let i = 0; i < allCards.length; i++) {
            allCards[i].classList.remove('active-card')
        }
        e.target.parentNode.classList.add('active-card')
    ...
    // first card have border...after randing playlist dynamically....before it no card available 
    ...
    videoViews.text(videoPlaySectionData[0].views / 1000 + 'k')
    videoTitle.text(videoPlaySectionData[0].title)
    plavideoDescription.text(videoPlaySectionData[0].description)
    if(videoPlaySectionData[0].isLiked == true){
        likeIcon.removeClass('far')
        likeIcon.addClass('fas')
    }else{
        likeIcon.removeClass('fas')
    ...
    if(videoPlaySectionData[0].isSaved == true){
        saveIcon.removeClass('far')
        saveIcon.addClass('fas')
    }else{
        saveIcon.removeClass('fas')
